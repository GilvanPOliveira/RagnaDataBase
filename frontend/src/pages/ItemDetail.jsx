    ...
    <div className="item-detail container">
      ...
      <div className="detail-header">
        ...
        <div className="stats">
          ...
          <p><strong>Preço NPC:</strong> Compra {item.buy_price_npc} / Venda {item.sell_price_npc}</p>
      ...
      <section className="sold-by">
        ...
        {item.sold_by.length > 0 ? (
          <ul>
            {item.sold_by.map((s, idx) => (
              <li key={idx}>
                {s.npc_name} ({s.map}) - Preço: {s.price}
              </li>
            ))}
          </ul>
        ) : (
        ...
        {item.drops.length > 0 ? (
          <ul>
            {item.drops.map((d, idx) => (
              <li key={idx}>
                {d.monster_name} ({d.map}) - Taxa: {d.drop_rate}%
              </li>
            ))}
          </ul>
        ) : (
